...
func Test_LogicalTypeGettersComprehensive(t *testing.T) {
	lt1 := NewLogicalType()
	require.Nil(t, lt1.GetSTRING())

	stringType := NewStringType()
	lt1.STRING = stringType
	require.Equal(t, stringType, lt1.GetSTRING())

	lt2 := NewLogicalType()
	require.Nil(t, lt2.GetMAP())
	...
	lt8 := NewLogicalType()
	require.Nil(t, lt8.GetTIMESTAMP())
	...
	lt10 := NewLogicalType()
	require.Nil(t, lt10.GetUNKNOWN())

	nullType := &NullType{}
	lt10.UNKNOWN = nullType
	require.Equal(t, nullType, lt10.GetUNKNOWN())

	lt11 := NewLogicalType()
	require.Nil(t, lt11.GetJSON())
...
func Test_TimeUnitGettersComprehensive(t *testing.T) {
	tu1 := NewTimeUnit()
	require.Nil(t, tu1.GetMILLIS())

	millis := NewMilliSeconds()
	tu1.MILLIS = millis
	require.Equal(t, millis, tu1.GetMILLIS())

	tu2 := NewTimeUnit()
	require.Nil(t, tu2.GetMICROS())

	micros := NewMicroSeconds()
	tu2.MICROS = micros
	require.Equal(t, micros, tu2.GetMICROS())

	tu3 := NewTimeUnit()
	require.Nil(t, tu3.GetNANOS())

	nanos := NewNanoSeconds()
	tu3.NANOS = nanos
	require.Equal(t, nanos, tu3.GetNANOS())
}

func Test_CountSetFieldsTimeUnitComprehensive(t *testing.T) {
	tests := []struct {
		name     string
		setupTU  func() *TimeUnit
		expected int
	}{
		{
			"no fields set",
...
func Test_ComprehensiveReadMethodsWithErrorHandling(t *testing.T) {
	...
	// Read it back to test all Read field paths
	...
	// Verify all fields were read correctly
	require.True(t, bytes.Equal(newStats.Max, stats.Max))
	...
	// Verify all fields were read correctly
	require.Equal(t, newSchemaElem.GetType(), schemaElem.GetType())
