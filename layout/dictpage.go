...
// DictRecToDictPage converts a dictionary record to a dictionary page using the provided options.
func DictRecToDictPage(dictRec *DictRecType, opt PageWriteOption) (*Page, int64, error) {
	var totSize int64 = 0
	...
	compressedData, err := page.dictPageCompress(dictRec.Type, compressor)
	if err != nil {
		return nil, 0, err
	}
...
func (page *Page) dictPageCompress(pT parquet.Type, compressor *compress.Compressor) ([]byte, error) {
...
// TableToDictDataPages converts a table to dictionary-encoded data pages using the provided options.
func TableToDictDataPages(dictRec *DictRecType, table *Table, bitWidth int32, opt PageWriteOption) ([]*Page, int64, error) {
	var totSize int64 = 0
	...
	for i < totalLn {
		...
		compressor := lookupCompressor(opt.Compressors, opt.CompressType)
		compressedData, compressErr := page.dictDataPageCompress(bitWidth, values, compressor)
		if compressErr != nil {
...
func (page *Page) dictDataPageCompress(bitWidth int32, values []int32, compressor *compress.Compressor) ([]byte, error) {
