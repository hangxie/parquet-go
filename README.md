...
`marshal.ConvertToJSONFriendly` reports any value its column cannot render, wrapping `types.ErrUnrenderable`, rather than returning the substitute `ConvertToJSONType` produces. That covers every case listed under [Reading Values](#reading-values): BSON that does not parse or is empty, bytes a geospatial rendering cannot be produced from, a `UUID`, `FLOAT16`, `INTERVAL` or `INT96` whose width is not the one the format fixes, an integer outside the range its annotation declares, and a value that is not bytes where bytes are required. Up to v3.8.3 each came back as a substitute indistinguishable from a value that rendered. The conversion stops at the first and returns no output, so one such value is reported for the whole batch rather than hidden in it; `types.ConvertToJSONType` still renders a single value best-effort if that is what a caller wants.
