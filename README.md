...
A column chunk may hold its pages in a file of its own, named by `ColumnChunk.file_path`, leaving the metadata file carrying nothing but the footer for that chunk. From v3.9.0 each column buffer keeps its handle on the metadata file for its whole life and opens a second handle for a chunk that names one, so a row group whose chunk names no file is read from the metadata file again. Up to v3.8.3 the metadata file's handle was closed and replaced by the external one, and every later row group was then read from that external file at offsets belonging to the metadata file, returning another file's rows or a read error. Consecutive row groups naming the same file share one handle, a path is resolved against the metadata file rather than against whichever external file was open, and both handles are closed when the reader is reset or stopped. Support is limited to pages: column indexes, offset indexes and bloom filters are still read from the metadata file, and a backend that cannot follow the path decides for itself what `Open` does with it.
