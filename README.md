...
- A column whose logical type has no defined sort order carries no `min`/`max` statistics: `INTERVAL`, whose ordering the specification leaves undefined, and `GEOMETRY`/`GEOGRAPHY`, for which `GeospatialStatistics` carries the bounds instead. Up to v3.8.3 a dictionary-encoded column of either kind wrote them anyway, from the raw bytes.
