...
Column chunk statistics: the writer reads each value's WKB to build the `GeospatialStatistics` bounding box and geometry type list. This library's reader is 2D, so a value it cannot read leaves both halves incomplete, and an incomplete statistic is worse than none: a bounding box smaller than the data makes a reader pushing a spatial filter down to it skip rows that match, and a short type list tells a reader the chunk holds only the types it could read. From v3.9.0, a column chunk holding an ISO geometry with Z or M ordinates carries no bounding box, and one holding bytes without a readable WKB header, or a value that is not bytes at all, carries neither a bounding box nor a type list. A value whose header reads but whose body does not, such as a truncated point, still contributes its type and withdraws only the box. Up to v3.8.3 the Z or M value was read as 2D, so the box was built from coordinates the value does not hold, and a value that could not be read was skipped, so the box covered the rest of the chunk as though it were all of it. A Z or M geometry still contributes its type, which its WKB header carries as plainly as a 2D one, and a page holding no geometries at all still leaves the chunk's box to the pages that do.
