...
`types.ConvertVariantValue` follows the same two-result convention. Invalid metadata or value bytes return a map containing the base64 of both byte arrays alongside an error wrapping `types.ErrUnrenderable`. The value byte array must contain exactly one complete VARIANT value: bytes after it, bytes left inside an array element or object field span, and invalid container offsets are errors. Object values may still be stored in a different physical order from their field IDs, as the format permits. For compatibility, a zero-length value retains its historical nil result without validating metadata; the Parquet VARIANT encoding of null is the one byte `0x00`, not an empty value. `EncodeVariantObject` and `EncodeVariantArray` include the zero final offset required for empty containers, producing `02 00 00` and `03 00 00`. The decoder also accepts the two-byte forms `02 00` and `03 00` emitted by earlier parquet-go releases; this exception does not apply to other container headers.

Two header fields moved to the bit positions the VARIANT encoding gives them in v3.9.0. Metadata carries `offset_size_minus_one` in bits 6-7 of its header byte, where releases up to v3.8.3 wrote it in bits 5-6, so an unsorted dictionary needing two-byte offsets now has header `41` rather than `21`. Object headers carry `field_id_size_minus_one` in bits 2-3 and `field_offset_size_minus_one` in bits 0-1, where those releases wrote the two the other way round: one-byte field IDs with two-byte offsets, which every object holding more than 255 bytes of values uses, now has value metadata `06` rather than `12`. Objects using one width throughout, which is most of them, read the same either way.

What those releases wrote is still read. No header bit tells the two layouts apart, and bit 5 of a metadata header is reserved and ignored here as the format requires, so they are told apart by where they end: metadata must end at the end of its dictionary, and an object at the end of the value handed to it, and a reading with the wrong widths stops somewhere else. The older widths are tried only once the conforming ones have failed that test. Metadata carrying bytes after its dictionary is reported for the same reason, as a value carrying bytes after it already was.
...
| `FLOAT16` | Go's 32-bit floating-point grammar, with surrounding whitespace allowed and the whole field consumed | `9.5abc` stored `9.5` |
...
`JSONWriter` takes the JSON form the column is written in, and from v3.9.0 reports the ones it is not. A column holding a number takes a JSON number: `{"n": "7"}` for an `INT64` is refused rather than parsed as text, as are the unsigned and signed integer annotations. `BOOLEAN` takes `true` or `false`, where `1` and `"true"` used to write `true` between them. `FLOAT` and `DOUBLE` additionally take the non-finite values in quotes, since JSON has no number for them: the reader emits `"NaN"`, `"Infinity"` and `"-Infinity"`, and any other spelling `strconv.ParseFloat` reads as non-finite, such as `"Inf"`, is taken as well. The annotations whose JSON form genuinely is a string are unaffected, because a string is what the reader produces for them and so what has to write back: `TIME`, `TIMESTAMP`, `INTERVAL`, `UUID`, `FLOAT16`, `DECIMAL`, the text and byte-backed columns, and `DATE` under its logical annotation. A column carrying only the legacy converted `DATE` reads back as its day count, a number, and takes either form. Errors name the column's physical type and the form supplied. Raw mode keeps its own grammar, described under [Value Modes](#value-modes).
...
An `INT96` column stores a nanosecond within the day followed by a Julian day, spanning 4713 BC to year 5874898. Both halves are carried whole: JSON output renders the value as an ISO 8601 timestamp with all nine fractional digits, and `JSONWriter`, `CSVWriter`, and `types.ParseINT96String` read that form back, including years that are negative or wider than four digits. A year has at least four digits after an optional minus sign, so `0999`, `-0123`, and `10000` are accepted while `999` and `-123` are not.

A timestamp the column cannot hold fails the write rather than being stored as a different one: a day past either end of the range gives an `outside the range an INT96 can hold` error, and a day that does not exist in the year supplied, such as `10001-02-29`, gives an `out of range for February` error. Input that is neither a timestamp nor a bare integer fails with a `parse INT96 timestamp` error; up to v3.8.3 it fell through to the integer form, so a rejected timestamp was stored as whatever digits it started with, and `5874898-06-04T00:00:00Z` became the number 5874898. The legacy bare-integer form accepts either sign and writes the whole value little-endian, but its magnitude must fit in 96 bits; larger positive or negative values are rejected rather than silently losing their high bits.
...
- Use `omitstats=true` in a field tag to skip statistics for large array fields. From v3.9.0 the column chunk then carries no `Statistics` at all, no `SizeStatistics`, and no column index — including a column whose pages are all null, whose index used to survive because all-null bounds are valid and would have published level histograms for it — and the writer skips the walk over every `BYTE_ARRAY` value that `unencoded_byte_array_data_bytes` needs, which is the cost the tag is reached for. Up to v3.8.3 it suppressed only `Statistics`, which was still written as an empty struct, while `SizeStatistics` was measured and published in full. Row counts and level histograms are still computed, since the offset index counts rows and null-page detection reads the definition-level histogram whether statistics are written or not.
...
- `OffsetIndex.unencoded_byte_array_data_bytes` carries that same measurement per page, from v3.9.0, so a reader pruning at page level sizes a buffer for the page it is about to read. The list covers every page of a chunk or is absent: an all-null page counts zero and keeps its slot, while a column of another type, or one tagged `omitstats`, carries no list at all.
...
Z and M ordinates: from v3.9.0 the readers take the dimension the type code declares, rather than assuming 2D.

| Value | GeoJSON and hybrid | Bounding box and size walk |
| --- | --- | --- |
| 2D | coordinates | measured |
| Z | coordinates with elevation, `[1, 2, 99]` | measured |
| M, ZM | `wkb_hex` substitute | measured |

RFC 7946 gives a position a third element for elevation and no place for a measure, so a Z geometry has a faithful GeoJSON form and M and ZM do not; rendering them as a position would drop the measure and read as an ordinary point. They are still measured, since the `GeospatialStatistics` bounding box is x and y at every dimension, and a `GeometryCollection` holding a Z or M member is read like any other. One dimension covers a geometry and every part of it, so a `Multi*` or `GeometryCollection` whose member declares another is refused by every reader here, as it already was by the write path. The GeoJSON write path takes the third ordinate back, so a Z geometry rendered as GeoJSON writes to the bytes it came from; a position of any other length, or one that disagrees with the rest of its geometry, is reported. One Z value keeps the `wkb_hex` substitute all the same: one carrying no position at all, such as an empty `LINESTRING Z`, whose GeoJSON is indistinguishable from an empty 2D geometry and would write back as one. Up to v3.8.3 the extra ordinates were read as though the value were 2D, so a `LINESTRING Z` over (1,2,99) (3,4,98) (5,6,97) came back as `[[1,2],[99,3],[4,98]]`. Two malformed shapes stay refused by the header check: a byte order byte other than 0 or 1, and a dimension above ZM, which is what an EWKB flag word such as PostGIS's `0x20000001` sets. The hex, base64 and raw modes carry any bytes and are unaffected.
...
Member headers: one structure walk measures a value for every reader here, so a member's own header is checked wherever a `GeometryCollection` is rendered as GeoJSON or added to a bounding box. Up to v3.8.3 the walk that stepped over a collection's members took a `MultiPoint` member to be the 21 bytes a 2D `Point` takes and skipped the header of a `MultiLineString` or `MultiPolygon` member unread, so a member of another type moved the walk to the wrong place and the members after it were read from there. From v3.9.0 such a value is refused, along with a member carrying a byte order byte other than 0 or 1. Only malformed values reach this: the format requires a `MultiPoint` to hold Points.

Nesting depth: every walk over a `GeometryCollection` recurses once per level, and a level costs nine bytes, so a crafted value could ask for as many stack frames as its length allows. From v3.9.0 all four walks — the GeoJSON rendering, the bounding box, the structure measure the write path validates with, and the GeoJSON-to-WKB encoder — refuse a value nesting collections more than 32 deep. Data does not nest anywhere near that far.
