...
type TypeList struct {
	...
	Int96             common.ByteArray `parquet:"name=int96, type=INT96"`
	Float             float32          `parquet:"name=float, type=FLOAT"`
	Double            float64          `parquet:"name=double, type=DOUBLE"`
	ByteArray         common.ByteArray `parquet:"name=bytearray, type=BYTE_ARRAY"`
	Enum              string           `parquet:"name=enum, type=BYTE_ARRAY, convertedtype=ENUM"`
	FixedLenByteArray common.ByteArray `parquet:"name=FixedLenByteArray, type=FIXED_LEN_BYTE_ARRAY, length=10"`
	...
	Interval         common.ByteArray `parquet:"name=interval, type=FIXED_LEN_BYTE_ARRAY, convertedtype=INTERVAL, length=12"`
	...
	Decimal3 common.ByteArray `parquet:"name=decimal3, type=FIXED_LEN_BYTE_ARRAY, convertedtype=DECIMAL, scale=2, precision=10, length=12"`
	Decimal4 common.ByteArray `parquet:"name=decimal4, type=BYTE_ARRAY, convertedtype=DECIMAL, scale=2, precision=20"`
...
func main() {
	...
	for i := range num {
		tp := TypeList{
			...
			Int32:             int32(i),
			Int64:             int64(i),
			Int96:             common.ByteArray(types.StrIntToBinary("12345", "LittleEndian", 12, true)),
			Float:             float32(float32(i) * 0.5),
			Double:            float64(float64(i) * 0.5),
			ByteArray:         common.ByteArray("ByteArray"),
			Enum:              "Enum" + strconv.Itoa(i),
			FixedLenByteArray: common.ByteArray("HelloWorld"),
			...
			Interval:        common.ByteArray(types.StrIntToBinary("12345", "LittleEndian", 12, false)),

			Decimal1: int32(12345),
			Decimal2: int64(12345),
			Decimal3: common.ByteArray(types.StrIntToBinary("-12345", "BigEndian", 12, true)),
			Decimal4: common.ByteArray(types.StrIntToBinary("12345", "BigEndian", 0, true)),
	...
	pr, err := reader.NewParquetReader(fr, new(TypeList), 1)
	if err != nil {
	...
	for i := 0; i < num; i++ {
		...
		tps[0].Decimal3 = common.ByteArray(types.DECIMAL_BYTE_ARRAY_ToString([]byte(tps[0].Decimal3), 10, 2))
		tps[0].Decimal4 = common.ByteArray(types.DECIMAL_BYTE_ARRAY_ToString([]byte(tps[0].Decimal4), 20, 2))
		log.Println(tps)
	}
	pr.ReadStop()
	_ = fr.Close()
