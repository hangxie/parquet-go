...
func NewAzBlobFileWriter(ctx context.Context, URL string, credential azcore.TokenCredential, clientOptions blockblob.ClientOptions) (source.ParquetFileReader, error) {
	var err error
...
func NewAzBlobFileWriterWithSharedKey(ctx context.Context, URL string, credential *blob.SharedKeyCredential, clientOptions blockblob.ClientOptions) (source.ParquetFileReader, error) {
	var err error
	var client *blockblob.Client
	if credential == nil {
		client, err = blockblob.NewClientWithNoCredential(URL, &clientOptions)
...
func NewAzBlobFileWriterWithClient(ctx context.Context, URL string, client *blockblob.Client) (source.ParquetFileReader, error) {
	if client == nil {
...
func (s *AzBlockBlob) Create(URL string) (source.ParquetFileReader, error) {
	var u *url.URL
	if len(URL) == 0 && s.URL != nil {
		// ColumnBuffer passes in an empty string for name
		u = s.URL
